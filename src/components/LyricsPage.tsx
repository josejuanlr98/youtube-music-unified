...
import { FaArrowLeft, FaExpand, FaMusic } from 'react-icons/fa';
...
export const LyricsPanel = ({ onBack, fullScreen = false }: LyricsPanelProps) => {
  ...
  const readerStyle = fullScreen
    ? { flex: '1 1 0', minHeight: 0, boxSizing: 'border-box' as const, padding: 'clamp(14px, 2.5vw, 32px)', overflowY: 'scroll' as const, overscrollBehavior: 'contain' as const, scrollBehavior: 'smooth' as const, fontSize: 'clamp(15px, 1.65vw, 24px)', lineHeight: 1.65, whiteSpace: 'pre-wrap' as const, overflowWrap: 'anywhere' as const }
  ...
  return (
    <Focusable className="ytm-ui" flow-children="vertical"
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexShrink: 0, alignItems: 'center', justifyContent: 'space-between', gap: 8, minWidth: 0 }}>
        <div style={{ minWidth: 0, textAlign: fullScreen ? 'center' : 'left', flex: 1 }}>{!fullScreen && <div className="ytm-eyebrow">Lyrics</div>}<div style={{ margin: '2px 0 0', fontSize: fullScreen ? 20 : 15, fontWeight: 700, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{fullScreen ? 'Now playing' : track?.title || 'Now playing'}</div></div>
        ...
        <DialogButton className="ytm-button" style={{ width: fullScreen ? 86 : 72, minWidth: 0, height: 28, minHeight: 28, padding: '0 8px', lineHeight: '28px', margin: 0, fontSize: 11 }} onClick={leave}><FaArrowLeft /> {fullScreen ? 'Exit' : 'Back'}</DialogButton>
      </div>
      <div className="ytm-lyrics-layout" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'row', gap: fullScreen ? 'clamp(18px, 4vw, 54px)' : 10, flex: 1, minHeight: 0, minWidth: 0, overflow: 'hidden', alignItems: fullScreen ? 'center' : 'stretch' }}>
        <div style={{ width: fullScreen ? 'min(26vw, 260px, calc(100vh - 260px))' : 78, minWidth: fullScreen ? 100 : 78, maxWidth: fullScreen ? '30%' : 78, flex: '0 0 auto', overflow: 'hidden', textAlign: fullScreen ? 'center' : 'left' }}>
          {track?.albumArt ? <img src={artwork} onError={event => { if (event.currentTarget.src !== track.albumArt) event.currentTarget.src = track.albumArt; }} alt="Album art" style={{ width: fullScreen ? '100%' : 78, height: fullScreen ? 'min(26vw, 260px, calc(100vh - 260px))' : 78, aspectRatio: '1', display: 'block', maxWidth: '100%', objectFit: 'cover', borderRadius: fullScreen ? 16 : 9, boxShadow: fullScreen ? '0 18px 48px rgba(0,0,0,.4)' : undefined }} />
            : <div className="ytm-card" style={{ width: '100%', aspectRatio: '1', display: 'grid', placeItems: 'center' }}><FaMusic size={fullScreen ? 54 : 26} /></div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: '1 1 0', alignSelf: 'stretch', minHeight: 0, minWidth: 0, overflow: 'hidden' }}>
          ...
          <div className="ytm-hint" style={{ fontSize: fullScreen ? 12 : 10, textAlign: 'center', width: '100%', display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '4px 8px' }}>
